...
# Define inline policy for the IAM role to access DynamoDB and CloudWatch
resource "aws_iam_role_policy" "lambda_for_lambda_policy" {
  name = "lambda_for_lambda_policy"                          # Name of the inline policy
  role = aws_iam_role.lambda_for_lambda.id                   # ID of the IAM role to attach the policy to
